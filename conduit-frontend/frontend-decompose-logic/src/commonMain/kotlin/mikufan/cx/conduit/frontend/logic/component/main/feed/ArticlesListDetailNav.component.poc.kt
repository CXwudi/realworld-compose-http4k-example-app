...
/**
 * PoC: ArticlesListDetailNav component using standard Decompose APIs
 * 
 * This demonstrates how the existing component can be modified to use standard 
 * Decompose childPanels() API with dynamic back handler registration instead of 
...
class DefaultArticlesListDetailNavComponentPoC(
        ...
        when {
          ...
          widestMode == ChildPanelsMode.DUAL && oldPanels.details != null -> oldPanels.copy(mode = ChildPanelsMode.DUAL)
            ...
            if (widestMode == ChildPanelsMode.TRIPLE) {
              log.warn { "Triple mode is not supported in this ArticlesListDetailNavComponentPoC" }
            }
            oldPanels
          }
        }
  ...
  override val panels: Value<ChildPanels<*, ArticlesListDetailNavComponentChild.ArticlesList,
      *, ArticlesListDetailNavComponentChild.ArticleDetail,
      Nothing, Nothing>> =
    // PoC: Using standard Decompose API with dynamic back handler
    standardChildPanelsWithBackHandler(
      source = panelNavigation,
      key = "ArticlesListDetailPanelPoC",
